...
class LPSwapParser:
    ...
    def parsing(self, target_img_path, filter_json_path, merge_result=True, need_align=True):
        """Main parsing and license plate swap function"""
        try:
            # Load filter information from filter.json
            with open(filter_json_path, "r") as filter_file:
            ...
            # Swap each detected license plate with respective source image based on filter
            origin_img_mat = img_mat.copy()
            for i, point_pred in enumerate(point_preds):
                if i < len(filters):
                    if filters[i]["consent"]:
                        filter_type = filters[i]["plateType"]
                        source_img = self.chooseRandomImage(filter_type)
...
def main():
    ...
    try:
        ...
        parser.parsing('./data/target/UPLOAD_IMG.png', 'filter.json')
